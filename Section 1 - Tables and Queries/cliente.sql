...
-- 8. Os clientes que não informaram o RG
...
-- 11. Os clientes que informaram o complemnto da residência
select nome,complemento from cliente where complemento is not null;

-- 12. Os clientes que moram em Santa Catarina - SC
select nome,uf from cliente where uf like 'SC';

-- 13. Os clientes que nasceram entre 01/01/1991 e 01/01/1998
select nome,data_nascimento from cliente where data_nascimento between '1991-01-01' and '1998-01-01';

-- 14. O nome do cliente e o logradouro, número, complemento, bairro, município e UF concatenado de todos os clientes
select 'Nome: '||nome||', logradouro :'||logradouro||', complemento :'||complemento||', bairro :'||bairro||', município :'||municipio||', uf :'|| uf from cliente;
select nome || ' - ' || logradouro || ' - ' || numero || ' - ' || complemento || ' - ' || bairro || ' - ' || municipio || ' - ' || uf from cliente;

select * from cliente;
-- 1. Alterar dados do cliente Maicon
...
values (17,'Getúlio','4631','Estudante', 'Brasileira', 'Rua Central', '343', 'Apartamento', 'Centro', 'Curitiba','SC');
...
-- 4.	Altere os dados da cliente Sandra
	...
	-- c.	O número para 123
update cliente set genero = 'F', profissao = 'Professora' where idcliente = 18;
...
left outer join 
	complemento as cpm on cln.idcomplemento = cpm.idcomplemento
left outer join 
...
-- 12. O nome da transportadora e o somatório do valor do pedido (agrupado por transpostadora). 
select
	trn.nome as transportadora,
	sum(pdd.valor) as total
from
...
SELECT 
    nome,
    COALESCE(TO_CHAR(data_nascimento, 'Month'), 'Não informado') AS mes_nascimento
FROM cliente;
...
SELECT 
    nome,
    COALESCE(TO_CHAR(data_nascimento, 'YYYY'), 'Não informado') AS ano_nascimento
FROM cliente;
...
-- 6. Atualizar o valor do pedido em 5% para os pedidos que o somatório do valor total dos produtos daquele pedido seja maior que a média do 
...
where 
	...
	select 
		avg(valor_unitario) from pedido_produto);
select
	pdd.idpedido,
